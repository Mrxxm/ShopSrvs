...
func (s *GoodsService) GetAllCategorysList(context.Context, *emptypb.Empty) (*proto.CategoryListResponse, error) {
	...
	*/
	...
	global.DB.Where(&model.Category{Level: 1}).Preload("SubCategory.SubCategory").Preload("SubCategory.PCategory").Find(&categories)
...
func (s *GoodsService) GetSubCategory(ctx context.Context, req *proto.CategoryListRequest) (*proto.SubCategoryListResponse, error) {
	...
	categoryListResponse.Info = &proto.CategoryInfoResponse{
		...
		ParentCategory: category.PCategoryID,
	}
	...
	//}
	global.DB.Where(&model.Category{PCategoryID: req.Id}).Find(&subCategorys)
	for _, subCategory := range subCategorys {
		subCategoryResponse = append(subCategoryResponse, &proto.CategoryInfoResponse{
			...
			IsTab:          category.IsTab == 1,
			ParentCategory: subCategory.PCategoryID,
		})
	}
...
func (s *GoodsService) UpdateCategory(ctx context.Context, req *proto.CategoryInfoRequest) (*emptypb.Empty, error) {
	...
	if req.ParentCategory != 0 {
		category.PCategoryID = req.ParentCategory
	}
	if req.Level != 0 {
		category.Level = req.Level
	}
	if req.IsTab {
		category.IsTab = int8(category.IsTab)
	}
