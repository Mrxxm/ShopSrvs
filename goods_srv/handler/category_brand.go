...
// 品牌分类
func (s *GoodsService) CategoryBrandList(context context.Context, req *proto.CategoryBrandFilterRequest) (*proto.CategoryBrandListResponse, error) {
	...
	for _, categoryBrand := range categoryBrands {
		categoryResponses = append(categoryResponses, &proto.CategoryBrandResponse{
			Category: &proto.CategoryInfoResponse{
				...
				IsTab:          categoryBrand.Category.IsTab == 1,
				ParentCategory: categoryBrand.Category.PCategoryID,
			},
...
// 通过category获取brands
func (s *GoodsService) GetCategoryBrandList(context context.Context, req *proto.CategoryInfoRequest) (*proto.BrandListResponse, error) {
	...
	var categoryBrands []model.GoodsCategoryBrand
	if result := global.DB.Where(&model.GoodsCategoryBrand{CategoryID: category.ID}).Find(&categoryBrands); result.RowsAffected > 0 {
